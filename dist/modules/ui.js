...
export function openMediaDetailsModal(itemId, store) {
    ...
    const starsHTML = Array.from({ length: 5 }, (_, i) => `<span class="modal-star${i < stars ? ' filled' : ''}">${i < stars ? '★' : '☆'}</span>`).join('');
    // Color del tipo
    ...
    const modalFavBtn = modalBody.querySelector('.modal-fav-toggle');
    if (modalFavBtn) {
        modalFavBtn.addEventListener('click', () => {
            ...
            if (iconEl)
                iconEl.textContent = isNowFav ? '❤️' : '🤍';
            if (labelEl)
                labelEl.textContent = isNowFav ? t.inFavorites : t.favBtnAdd;
