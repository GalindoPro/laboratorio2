...
export const MOCK_MOVIES_DATA: MovieDTO[] = [
    {
        id: 1,
 ...
 */
export async function fetchMoviesAPI(): Promise<MovieDTO[]> {
    try {
        ...
        const moviesDTO: MovieDTO[] = dataEs.results.map(movieEs => {
 ...
 */
export function fetchReviewsAPI(simulateError: boolean = false): Promise<ReviewDTO[]> {
 ...
 */
export function fetchAdsAPI(simulateError: boolean = false): Promise<AdDTO> {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (simulateError) {
